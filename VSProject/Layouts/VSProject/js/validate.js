...
var jVal = {
	'fullName' : function() {
		...
		if(ele.val().length < 6) {
			jVal.errors = true;
				nameInfo.removeClass('correct').addClass('error').html('姓名至少需要六个字符').show();
	...
	'birthDate' : function (){
		...
		if(!patt.test(ele.val())) {
			jVal.errors = true;
				birthInfo.removeClass('correct').addClass('error').html('请输入正确的日期格式').show();
				ele.removeClass('normal').addClass('wrong');					
		} else {
	...
	'gender' : function (){
		...
		if($('input[name="gender"]:checked').length === 0) {
			jVal.errors = true;
				genderInfo.removeClass('correct').addClass('error').html('尚未选择你的性别！').show();
				ele.removeClass('normal').addClass('wrong');		
		} else {
	...
	'vehicle' : function (){
		...
		if($('input[name="vehicle"]:checked').length <= 1) {
			jVal.errors = true;
				vehicleInfo.removeClass('correct').addClass('error').html('抱歉，你至少选择两项才行!').show();
				ele.removeClass('normal').addClass('wrong');		
		} else {
