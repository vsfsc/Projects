...
namespace VADocumentOnline.OfficeOnshow
{
    ...
    public class HttpHelper
    {
        ...
        /// <param name="mg">todo: describe mg parameter on GetDataGetHtml</param>
        /// <returns></returns>
        ...
        public static bool HttpDownLoadFiles(string uri, string filefullpath, int size = 1024)
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        using (FileStream fs = new FileStream(fileTempFullPath, FileMode.Append, FileAccess.Write, FileShare.ReadWrite))
                        {
                    ...
                    {
                        using (FileStream fs = new FileStream(fileTempFullPath, FileMode.Create))
                        {
                            byte[] buffer = new byte[512];
                            HttpWebRequest request = (HttpWebRequest)HttpWebRequest.Create(uri);
                            request.Timeout = 10000;
