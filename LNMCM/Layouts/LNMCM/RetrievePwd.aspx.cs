...
namespace LNMCM.Layouts.LNMCM
{
    public partial class RetrievePwd : LayoutsPageBase
    {
            ...
            SPSecurity.RunWithElevatedPrivileges(delegate()
            {
                ...
                if (adUser != null)
                {
                    ...
                    if (adUser.Properties.Contains("mail") && adUser.Properties["mail"].Count > 0)
                    {
                        ...
                        lblMsg.Text = "你在报名时所留邮箱为：" + initEmail.Substring(0, 3) + "***" + initEmail.Substring(initEmail.IndexOf("@"));
                    }
                    else
                        lblMsg.Text = "你在报名时未提交邮箱地址！";
                }
                else
                {
                    lblMsg.Text = "报名号不存在！";
        ...
        private void SendEmail(long operateId, string checkCode, string toMail)
        {
            ...
            txtContent += "<br/>请你在20分钟内完成找回密码操作。如果你已关闭了操作页面，请<a href='" + siteUrl + "/_layouts/15/LNMC/ResetPwd.aspx?sendID=" + operateId.ToString() + "'>点此继续</a>.";
